...
|R_MICRON_JMPO    | 8                 | 32 bits | `(S-IP)>>2`  | Relocates against the aligned 22 bit offset sutable for a jump instruction, writing the top 20 bits to the upper 20 bits of the instruction |
...
|R_MICRON_RELAXJMPOFF_PC32 | 34       | 96 bits | N/A | Hints that a link editor or other tool may convert a pointed-to code sequence that loads and jumps to a 32-bit pc relative address into one that performs a direct jump to a 22-bit aligned offset from the resulting instruction |
